            ...
            <Form.Item name="region" label="区域" rules={isDisabled ? [] : [{ required: true, message: '请选择区域!' }]}>
                <Select disabled={isDisabled}>
                    {props.regionList.map((region: any) => (
                        <Option disabled={checkRegionDisabled(region)} key={region.id} value={region.value}>
                            {region.title}
                        </Option>
                    ))}
                </Select>
            </Form.Item>
            <Form.Item name="roleId" label="角色" rules={[{ required: true, message: '请选角色!' }]}>
                <Select
                    onChange={value => {
                        if (value === 1) {
                            setIsDisabled(true)
                ...
                >
                    {props.roleList.map((role: any) => (
                        <Option disabled={checkRoleDisabled(role)} key={role.id} value={role.id}>
                            {role.roleName}
                        </Option>
                    ))}
                </Select>
